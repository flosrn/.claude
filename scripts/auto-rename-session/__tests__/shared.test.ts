...
describe("isRealUserMessage", () => {
	...
	it("should return false for skill prompt expansions", () => {
		...
		expect(isRealUserMessage("Some text with <quick_start> in it")).toBe(false);
		expect(isRealUserMessage("<parameters>some params</parameters>")).toBe(
			false,
		);
		expect(isRealUserMessage("Text containing <workflow> tag")).toBe(false);
	});
